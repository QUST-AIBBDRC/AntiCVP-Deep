...
fastas = readFasta.readFasta(r"Y-train.fa.txt")
kw={'path': r"PAAC",'train':r".txt",'order':'ARNDCQEGHILKMFPSTWYV'}


result=PAAC(fastas, lambdaValue=4, w=0.05, **kw)
data=np.matrix(result[1:])
data_PseAAC=pd.DataFrame(data=data[:,1:])
data_PseAAC.to_csv('Y-train.fa.csv')
