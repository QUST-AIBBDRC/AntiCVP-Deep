...
for train, test in skf.split(X,y):
    ...
    clf_list = clf_cnn.fit(X_train_cnn, to_categorical(y[train]),epochs=30)   
    ...
    tprs_cnn[-1][0] = 0.0
    roc_auc = auc(fpr, tpr)
    sepscore_cnn.append([acc, precision,npv, sensitivity, specificity, mcc,f1,f2, Gmean, roc_auc])

row=ytest.shape[0]
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('ytest_sum_CNN_KMeansSMOTE_Train_Set1_30.csv')

yscore_=yscore[np.array(range(1,row)),:]
yscore_sum = pd.DataFrame(data=yscore_)
yscore_sum.to_csv('yscore_sum_CNN_KMeansSMOTE_Train_Set1_30.csv')
...
data_csv.to_csv('CNN_KMeansSMOTE_Train_Set1_30.csv')
