...
def get_RNN_model(input_dim,out_dim):
    model = Sequential()
    model.add(Bidirectional(LSTM(int(input_dim/2), return_sequences=True),input_shape=(1,1165)))
...
for train, test in skf.split(X,y):
    ...
    clf_list = clf_rnn.fit(X_train_rnn, to_categorical(y[train]),epochs=20)
...
ytest_sum.to_csv('ytest_sum_Bilstm_DNN_KMeansSMOTE_Train_Set1_21.csv')

yscore_=yscore[np.array(range(1,row)),:]
yscore_sum = pd.DataFrame(data=yscore_)
yscore_sum.to_csv('yscore_sum_Bilstm_DNN_KMeansSMOTE_Train_Set1_21.csv')
...
data_csv.to_csv('Bilstm_DNN_KMeansSMOTE_Train_Set1_21.csv')
